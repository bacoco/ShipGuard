...
A loop aimed at "fix the failing webhook path" invents a comparison and approves everything.
...
violations that pass the comparison and fail review.
...
Keep only the lines the goal puts in play. A writing comparison does not need the app's port.
...
- For a UI comparison, the critic's evidence should come from `/sg-visual-run`, so screenshots are
...
- `/sg-mission-lock` still governs authority. This loop decides *how good*, never *how far* —
