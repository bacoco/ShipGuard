...
Claude Code reviewed PR #78 at commit `475d84a` from its diff, without running the skill or rechecking external sources. It considered candidates 1–7 sufficiently covered to avoid adding duplicate rules and recommended implementing candidate 8. That is a textual assessment, not behavioral proof. Candidate 8 is now included, along with an explicit default output location: the current project's root, or the current working directory when that root is unknown or no project applies. The review also suggested an English description; the maintainer subsequently required the entire skill and generated goals to be in English, with language adaptation for user-facing dialogue. A subsequent Claude Code comparison of the French and English skill found no material requirement lost in translation. These reviews had not tested interview behavior; the subsequent trial below addresses a limited part of it.

Further changes should clarify a concrete problem without duplication or a universal checklist imposed on every goal.

## Chat post-mortem and initial with/without trial

The maintainer requested that failures in the actual development conversation improve the skill. The observations below come from that conversation and model responses on September 8, 2026. They are not invented examples, and the operator's mistakes are not attributed to the skill as a demonstrated cause.

| Observed failure | Context and consequence | Targeted correction |
| --- | --- | --- |
| The operator began collecting all issues and PRs from both projects when the user meant the three recent GrillGoal contributions. | A reference was expanded beyond its conversational scope; the user corrected it and the broad collection was stopped. This occurred outside the skill trial. | Resolve short replies and references against the specific question or proposal they answer. |
| The interview asked when adding GrillGoal would count as complete, while the human wanted to determine whether it was useful. | The user corrected the purpose. Naming a possible addition had been treated as accepting that addition as the outcome. | Preserve the underlying decision, including whether to adopt a proposed solution at all. Restate a corrected purpose and retain still-applicable answers. |
| The interview proposed an evaluation goal before running the requested trial. | This would have substituted preparation of a later test for the actual test. The operator rejected that extra step. | Keep the goal about the selected underlying problem; do not introduce a goal about organizing the interview or evaluation unless requested. |
| With the frozen skill, the first controlled response offered design and implementation branches already excluded by the shared request. | The model waited for a human answer but spent a question on settled scope and previewed more questions. Waiting alone did not make the question useful. | Identify the unresolved decision before asking; reuse answers, exclude ruled-out options, and handle delegated means without new approval questions. |
| Both conditions claimed the parent's separately assigned post-mortem as their own work. | The shared turn-two annotation explicitly assigned that work to the parent, yet both responses called it their responsibility. The current skill had no equally explicit distinction between the goal writer, the future executor, and other participants. | Distinguish those assignments and preserve each actor's responsibilities and permissions. |

The controlled case is whether ShipGuard should add optional ambiguity hooks, using issue #77 as shared context. Two fresh Claude Code sessions used the same requested model (`claude-opus-5`), high effort, disabled tools/customizations, and the same request and source text. Only one received the skill frozen at commit `bac4419`. Usage metadata for both also lists Haiku; this is a comparison of those CLI sessions, not a claim of a single-model benchmark without auxiliary activity.

Without the skill, the first response asked two non-blocking questions and immediately produced a draft goal using explicitly labeled, unconfirmed assumptions. With the skill, it deferred writing but reopened excluded options. Neither observation establishes overall superiority. The shared issue already contains substantial guidance, so this tests the incremental value of the skill on that source-rich case. Subsequent real human clarification excludes the separate recommendations-on-request study from both conditions.

At turn two, the condition without the skill called its output a final goal after scope clarification, before human review of that goal. The frozen-skill condition still asked about the future investigator's access and whether empirical measurement would be permitted. Both also described the parent's separate post-mortem work as their own task, despite its stated ownership. These are observed response differences and remaining scope problems, not a completed comparison of accepted final goals.

The first two targeted replays of amended interview wording still listed an excluded implementation option. The amendment therefore makes the rule explicit even for an option labeled as requiring a fresh confirmation or offered for rejection. Claude Code's separate textual review found the core contract preserved but caught an unintended narrowing of note cleanup to purpose/scope corrections; cleanup was restored for every correction. Textual approval and a clearer instruction are not proof that the behavior is reliable.

In the third targeted replay, the model kept the question framed as a decision rather than assumed adoption, but still offered a prototype option that it described as exceeding the stated scope and added a secondary logistics question. The unwanted questioning is therefore not demonstrated to be eliminated. These are three successive amended prompts, not repeated runs of one version or a measured success rate; further changes should follow additional evidence rather than accumulating more prohibitions from this one case.

At turn three, both conditions received the same subsequent human steering and accepted parent recommendation, including the parent's completed read-only Loriq study. Each retained only its own earlier responses. These are fresh CLI invocations with reconstructed transcripts, not resumed vendor sessions. The condition without the skill switched from writing the selected goal to organizing and judging the comparison, asked for the other transcript, and proposed skill amendments. One amendment explicitly made silence a decision; it was rejected, as were its unsupported question limits. The frozen-skill condition stayed with the selected goal and presented a synthesis for confirmation. It still claimed the parent's post-mortem and proposed unconfirmed execution and engine requirements. This supports a narrower observation of better task retention in this turn, not overall superiority or an accepted final goal.

A separate diagnostic replay replaced only the skill prefix in the original skill-condition turn-two input with the current skill, including the responsibility distinction. It retained the original first response and shared clarification, without the turn-three steering. The replay still claimed ownership of the parent's post-mortem and offered an issue-publication option. The new wording is therefore not a demonstrated behavioral fix. This is one replay of the cumulatively revised skill, not an isolated estimate of the added sentence's effect or a new final-goal comparison. Parent/participant roles are represented as text in a reconstructed prompt; this limits conclusions about delegation in an actual multi-agent session.

The original comparison keeps its frozen skill. Changes prompted by the post-mortem are evaluated separately rather than silently substituted into that condition. No human answer or confirmation is simulated. This is a limited text-interaction trial, not a hook execution, installation test, complete final-goal comparison, or general reliability benchmark. The source-rich input and parent annotations limit attribution to the skill. The lessons change the generator; this chat history is not a required skill dependency or a generic mistakes catalog to copy into future goals.

### Primary outcome: execution of the generated goal

The maintainer clarified that the essential outcome is the result obtained by the AI that uses the generated goal, potentially a less capable model. Interview quality and goal readability are diagnostic observations, not substitutes for that downstream result.

After the human accepted the parent's common synthesis, both original conditions delivered goal text on turn four. The texts were extracted unchanged, including defects: the condition without the skill added an unrequested word target, fixed headings, two access attempts and an unavailable-human assumption; the condition with the frozen skill included the full ambiguity rule and explicit precedents but retained restrictive verdict forms and an ownership error in its surrounding response. Acceptance of the common synthesis does not endorse these additions. The extracted texts are pilot inputs, not a claim of fully conforming final goals.

The downstream pilot executes the same read-only ShipGuard investigation with each goal in fresh contexts on `claude-opus-5` and `claude-sonnet-5`, at high effort. Within each model pair, tools, source snapshots, and execution limits are the same; only the supplied goal differs. Executors receive neither the interview nor the generator's `SKILL.md`. They produce the requested investigation memo using read and web tools. No hook is implemented or executed. The primary comparison concerns the resulting recommendation, source checks, retained constraints, unsupported claims, and unresolved human decisions, with cost and latency reported separately. No conclusion about model capability or the skill's effectiveness follows from model branding.

This is one exploratory case with one run per cell and human-assisted goal preparation. It cannot establish reliability, performance on coding tasks, or readiness for a governed integration. A model's review of a memo does not replace human acceptance or actual measurements of hook effectiveness. Further changes to the generator must follow demonstrated downstream defects rather than an assumption that a less capable executor needs more rules.

All four downstream runs returned a memo with a successful CLI termination and no reported permission denial. Their traces contain only the allowed read/search/fetch tools. Successful termination is an observation about the run, not acceptance of the memo's conclusions.

| Executor | Goal generation condition | Elapsed seconds | Reported total USD | Memo words |
| --- | --- | ---: | ---: | ---: |
| `claude-opus-5` | Without GrillGoal | 233.37 | 1.057518 | 1,983 |
| `claude-opus-5` | With frozen GrillGoal | 369.97 | 1.649490 | 3,995 |
| `claude-sonnet-5` | Without GrillGoal | 249.88 | 0.6139674 | 1,689 |
| `claude-sonnet-5` | With frozen GrillGoal | 303.20 | 0.8469394 | 3,668 |

Costs are the CLI's reported totals, including auxiliary model activity; all runs also report Haiku usage. Elapsed time is wall time measured around the invocation. Neither lower cost nor a longer memo establishes better task performance. The shared local corpus included the original issue, the historical design note and hook source, current PR metadata, and official Anthropic/Codex documentation snapshots. Live public fetching was also available: some executors read the later public design note containing the initial interview-trial post-mortem. Thus their source inputs were equally available but their actual retrieval paths and evidence differed; this was not a blind fixed-corpus experiment. They were not supplied with the interview transcripts or the generator's skill as instructions.

An independent Claude Code review (`claude-opus-5`, tools disabled) received the two goal texts, four delivered memos, shared source snapshots, and the original task criteria, with executor models and generation conditions hidden behind labels. It found more explicit precedent verification and evidence-change reporting in both executions of the GrillGoal-generated goal, at greater cost and length. It found material defects in all four memos and did not support a superiority or cheaper-model substitution claim. Its review could not adjudicate every claim about live pages outside the supplied snapshots.

The parent's source check also rejected parts of that review. Both Opus executions, and then the reviewer, conflated an engine continuing after hook failure with the failure being recorded as a passing check. The supplied Codex documentation explicitly describes reporting the error while continuing; these are separate properties. Similarly, a machine-generated continuation represented as a user prompt does not itself constitute human agreement. The historical design note already states that a Stop continuation is not new human approval. These are risks to verify, not demonstrated impossibility of the proposal's requirements. The reviewer also called the 1,983-word baseline Opus memo over its goal's 800–2,000-word target; that criticism does not match the recorded whitespace word count and is not retained.

Other observed defects remain relevant: the Sonnet baseline claimed the supplied hook source proved operation for weeks, although source code alone does not establish runtime history; the Sonnet treatment reported Claude Code interruption handling as missing although the supplied Stop documentation states that Stop does not run on a user interrupt; and several outputs presented an already-documented continuation risk as a new discovery. The treatment's more explicit provenance labels did not eliminate overclaims. Reviewer agreement is not independent proof of a claim. Recommendations to add a universal checklist of hook-specific mechanics to the generic generator were rejected: those details belong only in an applicable task goal or its evaluation.

The supported result is improved coverage of some requested research in these two treatment outputs, not demonstrated overall usefulness or reliable execution by a less capable AI. The next substantive evaluation should use independently defined acceptance checks on representative downstream tasks, retaining both failures and valid alternative solutions. This pilot does not justify implementing ambiguity hooks or adopting GrillGoal as a mandatory step.
